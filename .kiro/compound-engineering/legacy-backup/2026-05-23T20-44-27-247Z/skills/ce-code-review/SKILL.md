...
1. **Run the harness's built-in code review.** If `$ARGUMENTS` contained a review target (PR number, GitHub URL, or branch name) after stripping recognized tokens, forward that target to the built-in. If no target was provided, run the bare command and let the built-in default to the current branch.
   - If you are Claude Code, run the the review skill tool, passing the target if present (e.g., the review skill 123`, the review skill <PR-URL>`, the review skill <branch>`); otherwise run bare the review skill.
...
3. **Programmatic callers bypass this short-circuit.** When `mode:autofix`, `mode:report-only`, or `mode:headless` is present, ignore quick intent and run the full multi-agent review. Skill-to-skill callers that want the lightweight pass should invoke the review skill (or the harness equivalent) directly rather than route through this short-circuit.
...
- **Never edit files or externalize work.** Do not write the tmp skill/compound-engineering/ce-code-review/<run-id>/`, do not file tickets, and do not commit, push, or create a PR.
...
- **Write a run artifact** under the tmp skill/compound-engineering/ce-code-review/<run-id>/` summarizing findings, applied fixes, and advisory outputs. Include the artifact path in the structured output.
...
Pass `{run_id}` to every persona sub-agent so they can write their full analysis to the tmp skill/compound-engineering/ce-code-review/{run_id}/{reviewer_name}.json`.
...
Each persona sub-agent writes full JSON (all schema fields) to the tmp skill/compound-engineering/ce-code-review/{run_id}/{reviewer_name}.json` and returns compact JSON with merge-tier fields only:
...
3. **Spawn validators with bounded parallelism.** One sub-agent per finding, dispatched independently using the validator template and the same bounded scheduler from Stage 4. Each validator receives:
   ...
   - `why_it_matters` when available — loaded from the per-agent artifact file at the tmp skill/compound-engineering/ce-code-review/{run_id}/{reviewer_name}.json`; omit when the file is absent or the artifact write failed. The validator proceeds without it, using the diff and cited code directly.
   - The full diff
...
Artifact: the tmp skill/compound-engineering/ce-code-review/<run-id>/
...
**Detail enrichment (headless only):** The headless envelope includes `Why:`, `Evidence:`, and `Suggested fix:` lines. After merge (Stage 5), read the per-agent artifact files from the tmp skill/compound-engineering/ce-code-review/{run_id}/` for only the findings that survived dedup and confidence gating.
...
- In interactive, autofix, and headless modes, write a per-run artifact under the tmp skill/compound-engineering/ce-code-review/<run-id>/` containing:
