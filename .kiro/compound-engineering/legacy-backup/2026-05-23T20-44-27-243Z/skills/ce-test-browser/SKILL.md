...
if lsof -i ":${PORT}" -sTCP:LISTEN -t >/dev/null 2>&1; then
  ...
  if [ "${PIPELINE_MODE}" = "1" ]; then
    ...
    if [ -f "bin/dev" ]; then
      PORT=${PORT} bin/dev > the tmp skill/dev-server-${PORT}.log 2>&1 &
    elif [ -f "bin/rails" ]; then
      bin/rails server -p ${PORT} > the tmp skill/dev-server-${PORT}.log 2>&1 &
    elif [ -f "package.json" ]; then
      PORT=${PORT} npm run dev > the tmp skill/dev-server-${PORT}.log 2>&1 &
    fi
    ...
    if ! lsof -i ":${PORT}" -sTCP:LISTEN -t >/dev/null 2>&1; then
      ...
      tail -20 the tmp skill/dev-server-${PORT}.log 2>/dev/null
      exit 1
    fi
  else
...
the ce-test-browser skill

# Test specific PR
the ce-test-browser skill 847

# Test specific branch
the ce-test-browser skill feature/new-dashboard

# Test on a specific port
the ce-test-browser skill --port 5000
```
