...
- **Bug-shaped prompt** (user describes broken behavior — "fix the bug where X", error message, regression, "doesn't work"). Surface `ce-debug` as a route-out option alongside continuing with `ce-plan` whenever the bug surface is reachable (in cwd OR named repo found at another local path). Stay in `ce-plan` silently when the named code can't be found anywhere local — paper-planning is the only useful output for unreachable surfaces.
  ...
  **When the bug is at another local path (not cwd):**
  ...
  The accessibility classification is conservative and may under-suggest in monorepos, dependency bugs, or after renames. Users can always invoke the ce-debug skill manually.

  **Headless mode**: skip the ce-debug suggestion menu entirely; default to continuing with the ce-plan skill (the user's explicit invocation). There is no synchronous user to resolve a route-out choice, and auto-routing to ce-debug would change the skill mid-flight without authorization.
...
Confirm and I'll proceed to research, drawing on this scope. (You can also redirect to the ce-brainstorm skill if this is bigger than you initially thought — I'll stop here and load it for you.)
````
...
- Use the use_subagent tool to delegate to the ce-repo-research-analyst agent: Scope: technology, architecture, patterns. {planning context summary}
- Use the use_subagent tool to delegate to the ce-learnings-researcher agent: planning context summary
Collect:
...
- Use the use_subagent tool to delegate to the ce-best-practices-researcher agent: planning context summary
- Use the use_subagent tool to delegate to the ce-framework-docs-researcher agent: planning context summary
...
- Use the use_subagent tool to delegate to the ce-spec-flow-analyzer agent: planning context summary, research findings
...
1. **Start the ce-work skill** (recommended) - Begin implementing this plan in the current session
...
- **Start the ce-work skill** — Invoke the `ce-work` skill via the platform's skill-invocation primitive (`Skill` in Claude Code, `Skill` in Codex, the equivalent on Gemini/Pi), passing the plan path as the skill argument. Do not merely tell the user to type the ce-work skill — fire the invocation now so the plan executes in this session.
...
- **Create Issue** — Detect the project tracker (`gh` for GitHub, `linear` for Linear) and create the issue from the plan file as described under "Issue Creation" in `references/plan-handoff.md`. After creation, display the issue URL and ask whether to proceed to the ce-work skill via the platform's blocking question tool.
- **Open in Proof (web app) — review and comment to iterate with the agent** — Load the `ce-proof` skill in HITL-review mode with the plan file as `source file`, the plan title as `doc title`, identity `ai:compound-engineering` / `Compound Engineering`, and recommended next step the ce-work skill. Then follow the post-HITL resync logic in `references/plan-handoff.md`, which handles the four `ce-proof` return statuses, re-runs `ce-doc-review` after material edits, and falls back gracefully on upload failure.
