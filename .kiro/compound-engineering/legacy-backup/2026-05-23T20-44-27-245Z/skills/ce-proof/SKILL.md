...
- **Display name (`name` on `POST the presence skill):** `Compound Engineering` — human-readable, shown in Proof's presence chips and comment-author badges.
...
**Wire-format reminder.** the api skill/agent/{slug}/ops` uses a top-level `type` field; the api skill/agent/{slug}/edit/v2` uses an `operations` array where each entry has `op`. Do not mix — sending `op` to the ops skill returns 422.

**Every mutation requires a `baseToken`.** Reuse the `mutationBase.token` from the most recent the state skill or the snapshot skill read — tokens don't go stale in seconds, and `STALE_BASE` is a recoverable error. On `BASE_TOKEN_REQUIRED` or `STALE_BASE`, re-read and retry once. Only do a pre-mutation read if no prior read has happened in this session. See the baseToken recipe in `references/hitl-review.md`.

the edit skill/v2` block refs are a separate concern: they can drift across revisions, so re-fetch the snapshot skill for fresh refs before a block edit if any writes have landed since your last snapshot.

**Retry discipline after mutation errors — verify before retrying.** An error response is not proof that nothing was written.

- `STALE_BASE`, `BASE_TOKEN_REQUIRED`, `MISSING_BASE`, `INVALID_BASE_TOKEN` — pre-commit, token-related. Re-read the state skill and retry once with the same payload and a fresh `baseToken`. A generic mutate helper can auto-retry these.
...
**Block-level edits via the edit skill/v2`** (separate endpoint, separate shape):
```bash
...
Read the snapshot skill to get stable block `ref` IDs. `operations` commits atomically — either every op lands or none do — so one the edit skill/v2` call can batch dozens of block edits safely and efficiently (see the bulk-sweep guidance in `references/hitl-review.md` Phase 2.4).

**Editing while a client is connected is fine.** the edit skill/v2`, `suggestion.add` (including `status: "accepted"`), and all comment ops work during active collab. Only `rewrite.apply` is blocked by `LIVE_CLIENTS_PRESENT` — it would clobber in-flight Yjs edits.
...
- Bridge-style endpoints (`/d/{slug}/bridge/*`) require client version headers (`x-proof-client-version`, `x-proof-client-build`, `x-proof-client-protocol`) and return 426 CLIENT_UPGRADE_REQUIRED without them. Use the api skill/agent/{slug}/ops` instead.
...
| POST | the marks skill/suggest-replace` | `{"quote":"old","by":"ai:compound-engineering","content":"new"}` |
| POST | the marks skill/suggest-insert` | `{"quote":"after this","by":"ai:compound-engineering","content":"insert"}` |
| POST | the marks skill/suggest-delete` | `{"quote":"delete this","by":"ai:compound-engineering"}` |
| POST | the marks skill/comment` | `{"quote":"text","by":"ai:compound-engineering","text":"comment"}` |
| POST | the marks skill/reply` | `{"markId":"<id>","by":"ai:compound-engineering","text":"reply"}` |
| POST | the marks skill/resolve` | `{"markId":"<id>","by":"ai:compound-engineering"}` |
| POST | the marks skill/accept` | `{"markId":"<id>"}` |
| POST | the marks skill/reject` | `{"markId":"<id>"}` |
| POST | the rewrite skill | `{"content":"full markdown","by":"ai:compound-engineering"}` |
...
- Reuse `baseToken` from your most recent the state skill or the snapshot skill read; on `STALE_BASE`, re-read and retry once
