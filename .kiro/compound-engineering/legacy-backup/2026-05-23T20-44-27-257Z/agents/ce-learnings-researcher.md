...
When no relevant learnings are found, say so explicitly, include the search context so the caller can see what was looked for, and note that the caller's work may be worth capturing with the ce-compound skill after it lands — the absence is itself useful signal.
...
- the ce-code-review skill, the ce-optimize skill, the ce-ideate skill — to surface prior learnings relevant to the change, optimization target, or ideation topic
