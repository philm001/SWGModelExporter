							...
							for (auto& modelIterator : listIterator)
							{
								...
								if (bad_shaders)
								{
									...
									if (safe_clear)
									{
										...
										for (size_t idx = 0; idx < modelIterator.getShaders().size(); ++idx)
										{
								...
								auto mesh_desc_it = referenceContext.object_list.find(openedByName);
								auto mesh_description = (mesh_desc_it != referenceContext.object_list.end())
									? std::dynamic_pointer_cast<Animated_object_descriptor>(mesh_desc_it->second)
									: nullptr;
								...
								for (auto it_skel = modelIterator.getSkeletonNames().begin(); it_skel != modelIterator.getSkeletonNames().end(); ++it_skel, ++skel_idx)
								{
									...
									if (mesh_description && skel_idx < mesh_description->get_skeletons_count())
									{
